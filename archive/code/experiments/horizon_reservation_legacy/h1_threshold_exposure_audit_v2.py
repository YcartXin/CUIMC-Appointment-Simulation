"""Second baseline-only threshold-exposure audit for revised patient profiles.
...
This gives 18 profiles x 90 clinic contexts = 1,620 backgrounds. The default
uses two seeds, for 3,240 baseline simulations.
...
DEFAULT_BANK = (
    ...
    / "outputs"
    / "hypotheses"
    / "h1_threshold_exposure_audit_v2_bank.csv"
)
DEFAULT_OUTPUT_DIR = (
    REPO_DIR
    / "outputs"
    / "hypotheses"
    / "h1_threshold_exposure_audit_v2"
)
...
def patient_profiles() -> list[AuditProfile]:
    """Return the revised profiles for the second threshold audit."""

    profiles: list[AuditProfile] = []

    # No-show family:
    # * 14 days is retained only as the low-sensitivity control.
    # * 7, 5, and 3 days test increasingly exposed thresholds.
    noshow_references = {
        "low": {
            "class2": (14, 0.05, 0.15),
            "class1": {
    ...
    # Balking family:
    # * 16 days is retained only as the low-sensitivity control.
    # * 6, 5, and 4 days are the main exposure candidates.
    balk_references = {
        "low": {
            "class2": (16, 0.05, 0.15),
            "class1": {
    ...
    # The repository requires the balking threshold to exceed the no-show
    # threshold. A 3-day fixed no-show threshold preserves that ordering for
    # the 4-, 5-, 6-, and 16-day balking candidates.
    ...
    # Joint profiles require balking thresholds to remain above no-show
    # thresholds. The low family retains 14/16 as a control, tests 7/9 as an
    # intermediate pair, and tests 3/4 as the strong pair. The moderate
    # family focuses on the 5/6, 4/5, and 3/4 transition region.
    joint_references = {
        "low": {
        ...
        "moderate": {
            "class2_noshow": (5, 0.05, 0.20),
            "class2_balk": (6, 0.05, 0.20),
            "class1": {
                "same": {
                    "noshow": (5, 0.05, 0.20),
                    "balk": (6, 0.05, 0.20),
...
def generate_bank() -> pd.DataFrame:
            ...
            rows.append(
                {
                    ...
                    "design_note": "threshold_exposure_audit_v2",
                    **profile_record,
                    **context,
    ...
    if len(bank) != 1620:
        raise AssertionError(f"Expected 1,620 backgrounds, found {len(bank)}")
...
def run_audit(
    *,
    ...
    if smoke:
        smoke_profiles = ["NS_MODERATE_MILD", "BK_MODERATE_MILD", "JT_MODERATE_STRONG"]
        bank = bank[
            bank["profile_id"].isin(smoke_profiles)
            & bank["rho"].isin([1.2, 2.0, 3.0])
...
def _profile_recommendations(flags: pd.DataFrame) -> pd.DataFrame:
    ...
    for keys, group in flags.groupby(group_cols, sort=False, dropna=False):
        ...
        is_low_control = (
            str(group["class2_reference"].iloc[0]) == "low"
            and str(group["contrast_level"].iloc[0]) == "same"
        )
        if is_low_control:
            recommendation = "retain as low-sensitivity control"
        elif informative_levels >= 2:
...
def _write_markdown_summary(
    *,
    ...
    lines = [
        "# Threshold Exposure Audit — Second Design",
        "",
