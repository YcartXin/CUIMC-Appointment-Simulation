"""Run the confirmatory controlled horizon-reservation experiment.
...
DEFAULT_BANK = REPO_DIR / "outputs" / "hypotheses" / "h1_patient_characteristics_full_bank.csv"
DEFAULT_OUTPUT = Path("/scratch") / "unknown" / "h1_patient_characteristics_full"

HORIZON_VALUES = tuple(range(2, 27, 2))
Q_COARSE_STEP = 5
...
def window_coarse_grid(horizon: int) -> list[int]:
    values = list(range(1, int(horizon) + 1, WINDOW_COARSE_STEP))
    if values[-1] != int(horizon):
...
def window_fine_grid(best_window: int, horizon: int) -> list[int]:
    ...
    hi = min(int(horizon), int(best_window) + WINDOW_REFINE_RADIUS)
