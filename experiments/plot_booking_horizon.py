...
Reads the summary CSVs and produces, for each objective:
    1a/1b  Objective heatmap over (H1, H2) — equal and Class-1-priority weights
...
OBJECTIVES = {
    ...
    "slot_util": dict(
        ...
        name="Objective - Slot Utilization", formula="w\u2081(Y\u2081/S) + w\u2082(Y\u2082/S)"),
}
...
def fig1_equal_priority(obj, cfg, fdir):
    hv = h_axis(obj)
    for tag, w1, label in [("equal", 1.0, "Equal weights (w\u2081=1)"),
                           ("priority", 2.0, "Class-1 priority (w\u2081=2)")]:
        sub = base_slice(obj, 9, 9, BASELINE_BALK, BASELINE_LAMBDA, w1)
        fig, ax = plt.subplots(figsize=(6, 5))
        im = draw(ax, matrix(sub, cfg["mean"], hv), hv, label,
                  annotate=".3f" if len(hv) <= 5 else None)
        fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
        fig.suptitle(f"{cfg['name']}: {cfg['formula']}\n"
                     f"\u03c4=(9,9), balk={BASELINE_BALK}, \u03bb={BASELINE_LAMBDA}",
                     fontsize=11, fontweight="bold")
        fig.tight_layout()
        name = "fig1a_objective_equal.png" if tag == "equal" else "fig1b_objective_priority.png"
        _save(fig, fdir / name)
