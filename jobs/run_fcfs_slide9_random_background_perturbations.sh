...
STUDY="${SLIDE9_RANDOM_STUDY:?SLIDE9_RANDOM_STUDY must be smoke or full}"
...
case "$STUDY" in
  smoke|full) ;;
  *) echo "Invalid SLIDE9_RANDOM_STUDY: $STUDY" >&2; exit 2 ;;
esac
...
cd "$REPO"
"$PYTHON" -u experiments/fcfs_slide9_random_background_perturbations.py \
  --mode run \
  --study "$STUDY" \
