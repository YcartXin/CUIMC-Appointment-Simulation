...
\begin{itemize}[nosep]
    ...
    \item \textbf{Class-1-first backfill reservation:} all Class 1 arrivals on a simulation day are handled before Class 2 arrivals; Class 1 searches reserved then general capacity on each residual day, and Class 2 may later backfill unused reserved slots.
\end{itemize}
...
\midrule
Reserved pool & \(Q=10\) & Class 1 always; Class 2 only under the Class-1-first backfill policy after that day's Class 1 arrivals have been handled \\
\addlinespace
...
\subsection{Class-1-First Backfill Reservation}

The Class-1-first backfill policy keeps the same \(Q=10\) protected slots, but it changes the
within-day order of the booking step. Arrivals on day \(D\) are handled in two priority stages.
First, all Class 1 arrivals are offered appointments. Second, Class 2 arrivals are offered
appointments. Class 2 cannot use protected capacity until the Class 1 stage for that simulation day
is complete.

\subsubsection{Stage 1: Class 1 Arrivals}

Let the randomized arrival sequence on day \(D\) be
\[
A^D=(a_1^D,a_2^D,\dots,a_{M^D}^D),
\]
where each \(a_k^D\in\mathcal I\). In the first stage, the policy processes only entries with
\[
a_k^D=1.
\]

For each Class 1 arrival, residual days are searched chronologically. Within each residual day,
reserved capacity is checked before general capacity:
\[
\tau_{1,\mathrm{bf}}
=
\min\left\{r\in\mathcal R:R_r^D<Q\ \text{or}\ G_r^D<S-Q\right\}.
...
Thus the Class 1 search order is
\[
(0,\mathrm{res}),\ (0,\mathrm{gen}),\ (1,\mathrm{res}),\ (1,\mathrm{gen}),\dots
\]
If both pools are unavailable for every \(r\in\mathcal R\), the Class 1 patient receives no offer.

\begin{keybox}[title={No Horizon-Wide Reserved-Only Pass}]
Class-1-first backfill reservation does not search all future reserved slots before general slots. If day 0 has no
reserved capacity but has a general slot, and day 1 has a reserved slot, a Class 1 patient is offered
the day 0 general slot.
\end{keybox}

\subsubsection{Stage 2: Class 2 Arrivals}

After all Class 1 arrivals from day \(D\) have been handled, the policy processes the Class 2
arrivals from that same simulation day.

Class 2 first searches the general pool and then may use leftover reserved capacity:
\[
\tau_{2,\mathrm{bf}}
=
...
Thus the Class 2 search order is
\[
(0,\mathrm{gen}),\ (0,\mathrm{res}),\ (1,\mathrm{gen}),\ (1,\mathrm{res}),\dots
\]
where a reserved slot is available to Class 2 only because the Class 1 stage for day \(D\) has already
ended.

\begin{keybox}[title={Class-1-First Backfill Interpretation}]
Class-1-first backfill reservation is a compromise. Class 1 receives priority access to \(Q\) protected daily slots,
but unused protected capacity can still be converted into Class 2 access after the day-level Class 1
stage.
\end{keybox}
...
Under Class-1-first backfill reservation after the day-level Class 1 stage, a Class 2 patient can use one of the three
...
\addlinespace
Class-1-first backfill reservation, Class 2 after Class 1 stage & \(r=1\), reserved if unused & leftover protected slots can be backfilled \\
\bottomrule
\end{tabular}
\normalsize
\end{center}
...
\subsection{Class-1-First Backfill Reservation Day}

Under Class-1-first backfill reservation, the day has two priority stages.

\begin{enumerate}[itemsep=0.35em]
    \item \textbf{Class 1 stage.} Class 1 arrivals search residual days chronologically and check reserved capacity before general capacity on each residual day:
    \[
    (0,\mathrm{res}),\ (0,\mathrm{gen}),\ (1,\mathrm{res}),\ (1,\mathrm{gen}),\dots
    \]
    Equivalently,
    \[
    \mathcal F_{1,\mathrm{bf}}^D
    =
    \left\{(r,\mathrm{res}):r\in\mathcal R,\ R_r^D<Q\right\}
    \cup
    \left\{(r,\mathrm{gen}):r\in\mathcal R,\ G_r^D<S-Q\right\},
    \]
    with the ordered search above. This is not a horizon-wide reserved-only pass.
    \item \textbf{Class 2 stage.} After all Class 1 arrivals have been handled, Class 2 arrivals search general capacity first and then leftover protected capacity on each residual day:
    \[
    (0,\mathrm{gen}),\ (0,\mathrm{res}),\ (1,\mathrm{gen}),\ (1,\mathrm{res}),\dots
    \]
    Equivalently,
    \[
    \mathcal F_{2,\mathrm{bf}}^D
    =
    \left\{(r,\mathrm{gen}):r\in\mathcal R,\ G_r^D<S-Q\right\}
    \cup
    \left\{(r,\mathrm{res}):r\in\mathcal R,\ R_r^D<Q\right\},
    \]
    with the ordered search above.
\end{enumerate}

This staged rule is the mathematical version of ``Class 1 gets priority within the day; if protected
slots remain after that Class 1 stage, Class 2 may fill them later.''
...
\toprule
\textbf{Feature} & \textbf{Pooled FCFS} & \textbf{Strict Class 1 reservation} & \textbf{Class-1-first backfill reservation} \\
\midrule
Capacity structure & all \(S\) slots pooled & \(Q\) protected, \(S-Q\) general & \(Q\) protected first, then releasable \\
\addlinespace
Class 1 access & competes only by arrival order & protected first, then general overflow & handled before Class 2; protected then general on each residual day \\
\addlinespace
Class 2 access & can use any open slot & can use only general slots & can use general slots and unused protected slots after the Class 1 stage \\
\addlinespace
...
slots are not filled. Class-1-first backfill reservation tests whether most of the Class 1 access protection can be
...
\addlinespace
Class-1-first backfill reservation, Class 1 & no day has either \(R_r^D<Q\) or \(G_r^D<S-Q\) during the Class 1 stage \\
\addlinespace
Class-1-first backfill reservation, Class 2 & no general slot and no leftover protected slot are available after the Class 1 stage \\
\bottomrule
\end{tabular}
\normalsize
\end{center}
...
\addlinespace
\texttt{release\_reserved\_slots} & backfill flag & false for strict reservation, true for Class-1-first backfill reservation \\
\addlinespace
...
\begin{itemize}[nosep]
    ...
    \item \texttt{process\_daily\_arrivals} applies either the one-pass rule or the Class-1-first backfill priority-stage rule.
    \item In the backfill rule, all Class 1 arrivals are processed before non-reserved classes.
    \item For Class 1, \texttt{find\_earliest\_open\_day} searches each residual day as reserved then general.
    \item For Class 2 in the backfill stage, the search order is general then leftover reserved on each residual day.
\end{itemize}
...
\begin{itemize}[nosep]
    ...
    \item In Class-1-first backfill reservation, Class 1 gets the day-level priority stage and Class 2 can backfill unused protected capacity afterward.
...
G_r^D<S-Q.
\]
Class-1-first backfill reservation keeps a protected day-level priority stage for Class 1, then relaxes the strict
rule by letting Class 2 use protected capacity that Class 1 did not reserve.
