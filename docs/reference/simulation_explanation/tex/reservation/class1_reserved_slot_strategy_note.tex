...
some protected capacity unused. A single metric is therefore not enough to decide whether a given
\(Q\) is attractive.
...
\subsubsection{Weighted Slot Use}

For class-specific slot use, define each class contribution to attended utilization as
\[
...
The weighted slot-use objective is
\[
U_{\mathrm{slot}}(w_1,w_2)
=
w_1\rho_1^{\mathrm{attended}}
+
w_2\rho_2^{\mathrm{attended}}.
\]
Class 2 is kept at \(w_2=1\). Increasing \(w_1\) makes Class 1 served slots count more in the
comparison. When \(w_1=w_2=1\), this reduces to total attended utilization
\(\rho^{\mathrm{attended}}\).
...
tradeoff, the net priority objective subtracts a cost for configured protected capacity:
\[
U_{\mathrm{net}}(w_1,w_2,c)
=
U_{\mathrm{served}}(w_1,w_2)
-
c\frac{Q}{S}.
...
harder to justify unless they create enough weighted service benefit.
...
\[
\rho^{\mathrm{attended}}\ge \rho_{\min},
\qquad
\min_i\ \mathrm{served\ rate}_i\ge x,
\qquad
\left|\rho_1^{\mathrm{attended}}-\rho_2^{\mathrm{attended}}\right|\le g.
\]
Here \(\rho_{\min}\) is the minimum attended utilization level, \(x\) is the minimum served-rate
floor for each class, and \(g\) is the largest allowed gap between the two class-specific attended
