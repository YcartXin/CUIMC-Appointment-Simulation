...
Cancellation probability graph: higher cancellation probability for class 1
slightly increases average utilization. This is because cancellations shorten
the booking delay and sometimes allow the slot to be before the no-show threshold.
...
The right graph shows that class 1 has a -0.15 difference between class 1 and 2
when class 1 has 1 no-show probability, and vice versa with 0.15 difference.


![No-show threshold interaction](figures/no_show_threshold_interaction_heatmap.png)
In the right graph, we see that the top right corner indicates that both classes
behave very similarly with high no-show threshold. This is because when the
threshold is high, the differentiating no-show days is very limited.
...
It is the same for class 1 & 2 because balking does not effect the offered delay.
With a small balking rate, the mean offered booking rises slightly because the
later slots are offered more repeatedly.
...
The balking-rate plot confirms that the balking rate rises the fastest in the
beginning. Class 2 balking rate increases at first because class 1 balk far
slots which class 2 is also likely to balk on.
...
1 tolerates longer waits before high balking applies. There is a steep drop at
13 because it turns off balking for the simulation.
