...
5. `05_reserved_slot_strategy.ipynb` compares strict Class 1 reservation with Class-1-first backfill reservation.
6. `06_reserved_slot_visual_walkthrough.ipynb` shows the same arrivals under pooled, strict reservation, and Class-1-first backfill reservation with slot-grid allocation views.
7. `07_reservation_policy_demand_sensitivity.ipynb` compares corrected strict and Class-1-first backfill reservation policies across symmetric and concentrated demand regimes.
8. `08_reserved_capacity_sensitivity.ipynb` studies how the corrected reservation policies change as reserved capacity `Q` varies.
